    ...
    fn accept(mut stream: service::RdpStream<'_>) -> Result<(), io::Error> {
        ...
        match cmd {
            protocol::DataCommand::Cwd(path) => {
                ...
                Self::cmd_cwd(&mut stream, path)?;
            }
            protocol::DataCommand::Dele(path) => {
                #[cfg(target_os = "windows")]
                let path = "C:".to_string() + path.as_str();

                Self::cmd_dele(&mut stream, path)?;
            }
            protocol::DataCommand::List(path) => {
                #[cfg(target_os = "windows")]
                let path = "C:".to_string() + path.as_str();

                Self::cmd_list(&mut stream, path)?;
            }
            protocol::DataCommand::NLst(path) => {
                #[cfg(target_os = "windows")]
                let path = "C:".to_string() + path.as_str();

                Self::cmd_nlst(&mut stream, path)?;
            }
            protocol::DataCommand::Retr(path) => {
                #[cfg(target_os = "windows")]
                let path = "C:".to_string() + path.as_str();

                Self::cmd_retr(&mut stream, path)?;
            }
            protocol::DataCommand::Size(path) => {
                #[cfg(target_os = "windows")]
                let path = "C:".to_string() + path.as_str();

                Self::cmd_size(&mut stream, path)?;
            }
            protocol::DataCommand::Stor(path) => {
                #[cfg(target_os = "windows")]
                let path = "C:".to_string() + path.as_str();

                Self::cmd_stor(&mut stream, path)?;
            }
        }
